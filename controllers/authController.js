...
const authController = {
  ...
  checkAccountExist: async (req, res, next) => {
    ...
    handleSuccess(res, null, "此 email 可以使用");
  },

  // 檢查帳號是否重複 (即已存在)
  checkAccountDuplicate: async (req, res, next) => {
    let { account } = req.body;
    ...
    const isAccountExist = await Member.findOne({ account: account });
    if (!isAccountExist) {
      return next(appError(400, "此 email 未註冊過會員"));
    }

    handleSuccess(res, null, "此 email 已註冊過會員");
  },
  ...
  login: async (req, res, next) => {
    ...
    // 檢查帳號是否存在且未被停權
    const member = await Member.findOne({ account: account }).select(
      "+password"
    );
  ...
  sendResetPwdEmail: async (req, res, next) => {
    ...
    const resetPasswordUrl = `http://localhost:5173/member/forgetPwd?token=${hashedToken}`;
    ...
    // 發送郵件
    const mailOptions = {
      from: "食享 tastejoy <yu13142013@gmail.com>",
      to: member.account,
      subject: `食享 tastejoy 找回您的密碼，繼續踏上美食之旅吧！`,
  ...
  googleLoginCallback: async (req, res, next) => {
    ...
    res.redirect(
      `https://ciaocraft-website.vercel.app/SSOLogin?user=${token}&memberId=${member._id}`
    );
  },
  ...
  unlinkGoogleAccount: async (req, res, next) => {
    ...
    // 清除 googleId
    const member = await Member.findByIdAndUpdate(memeberId, {
      googleId: null,
      googleAccount: null,
    });
